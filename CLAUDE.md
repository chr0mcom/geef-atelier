...
**Skeleton Schritt 6 abgeschlossen (10. Mai 2026).** Application-Service-Layer: neues Projekt `Geef.Atelier.Application` mit `IRunService` (Submit/Get/List/Cancel) und `RunService` (internal sealed, Scoped). `IRunRepository` (Core) + `RunRepository` (Infrastructure, internal sealed, Variante β — Application hat keine Infrastructure-Dep). `RunEntity.CancellationRequested bool`-Flag + EF-Migration `Step06Cancellation`. `OrchestratorOptions.CancellationPollingInterval` (1s). Cancellation-Watcher Pattern A (pro-Run-Task, pollt DB, signalisiert CTS) + erweiterter Catch-Block `when (cts.IsCancellationRequested)` → `OverrideToAbortedAsync("Cancelled by user")`. 31/31 Tests grün (5 neue Application-Tests). Parallel läuft M1 (OpenAI-kompatibler Provider) auf Branch `feature/openai-compatible-providers` — sollte vor Schritt 7 in main gemerged werden. Nächster Schritt: Schritt 7 — Blazor-UI mit `IRunService`-Aufrufen + SignalR-Live-Status.

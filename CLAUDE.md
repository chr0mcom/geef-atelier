...
**PDF-Support deployed (14. Mai 2026).** PS-1 ✅ PS-2 ✅ PS-3 ✅ PS-4 ✅ PS-5 ✅ PS-6 ✅ PS-7 ✅ + CLI-Split ✅ + Model-Catalog ✅ + Grounding-Viz ✅ + Vector-Store-RAG ✅ + Run-Attachments ✅ + PDF-Support ✅. PDF via PdfPig 0.1.14 (max 25 MB), `PdfTextExtractor` (nie werfend, `PdfExtractionResult`), `KnowledgeOptions.MaxPdfSizeBytes`, `FileDropZone` per-Typ-Limit, 519 Tests grün. D-037: `KnowledgeScope`-Enum (Global/RunLocal), `KnowledgeDocument.RunId` + FK mit `ON DELETE CASCADE`, `SystemCrew.RunAttachmentsProfile` (ProviderType="vector-store", Scope="run-local"), `SubmitRunRequest`-Record, `FileDropZone`-Komponente, `RunAttachmentsList`, `PromoteAttachmentModal`, `GroundingSection` Citation-Badges. 494 C#-Tests grün, 43 Python-Tests grün. D-036: `VectorStoreGroundingProvider`, `OpenRouterEmbeddingProvider`, `RecursiveCharacterTextSplitter`, pgvector 0.8.2. Postgres-Image: `pgvector/pgvector:pg16`.

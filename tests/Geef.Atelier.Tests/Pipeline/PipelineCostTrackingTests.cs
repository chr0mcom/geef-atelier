...
public sealed class PipelineCostTrackingTests
{
    ...
    private static readonly string ExecutorModel = SystemCrew.DefaultExecutorProfile.Model;
    ...
    public async Task Pipeline_WithCostAccumulator_RecordsExecutorCost()
    {
        ...
        // Executor model is from the profile (known model → cost not null)
    ...
    {
        // Build a catalog with only "known-model", but profiles use the system executor model
        var accumulator = new RunCostAccumulator();
